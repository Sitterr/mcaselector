...
import net.querz.mcaselector.version.anvil119.Anvil119ColorMapping;
...
public class Anvil120ColorMapping implements ColorMapping {

	private static final Logger LOGGER = LogManager.getLogger(Anvil120ColorMapping.class);
	...
	public Anvil120ColorMapping() {
		// note_block:pitch=1,powered=true,instrument=flute;01ab9f
		try (BufferedReader bis = new BufferedReader(
			new InputStreamReader(Objects.requireNonNull(Anvil120ColorMapping.class.getClassLoader().getResourceAsStream("mapping/120/colors.txt"))))) {
			String line;
			...
			throw new RuntimeException("failed to read mapping/120/colors.txt");
		}

		try (BufferedReader bis = new BufferedReader(
			new InputStreamReader(Objects.requireNonNull(ColorMapping.class.getClassLoader().getResourceAsStream("mapping/120/biome_colors.txt"))))) {
			...
			throw new RuntimeException("failed to read mapping/120/biome_colors.txt");
		}
	}
	...
	public int getRGB(Object o, int biome) {
		throw new UnsupportedOperationException("color mapping for 1.20 does not support numeric biome ids");
	}
